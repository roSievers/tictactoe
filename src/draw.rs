...
impl GraphicsCache {
    pub fn new(ctx: &mut Context) -> GameResult<Self> {
        ...
        let grid_offset = Vector2::new( width as f32 - height as f32 + height as f32 * 0.03, height as f32 * 0.03);
...
pub fn board(ctx: &mut Context, state: &mut MainState) -> GameResult<()> {
    ...
    match state.current_player {
        board::Player::Cross => cross(ctx, info_offset, state.gfx.measures.inner.get_block_size_without_padding(), state.gfx.measures.inner.line_width)?,
        board::Player::Circle => circle(ctx, &state.gfx.small_circle, info_offset, state.gfx.measures.inner.get_block_size_without_padding())?
    };
    ...
    for region in coord::Local::iter() {
        let region_offset = state.gfx.grid_offset + state.gfx.measures.outer.get_offset_with_padding(region);
        ...
        match board_region.total {
            board::Ownership::Undecided => {
                ...
                for local in coord::Local::iter() {
                    let token_offset = region_offset + state.gfx.measures.inner.get_offset_with_padding(local);
                    ...
                    draw_token(ctx, token, &state.gfx.small_circle, token_offset, state.gfx.measures.inner.get_block_size_without_padding(), state.gfx.measures.inner.line_width)?;
                }
            },
            board::Ownership::Cross => {
                cross(ctx, region_offset, state.gfx.measures.outer.get_block_size_without_padding(), state.gfx.measures.outer.line_width)?;
            },
            board::Ownership::Circle => {
                circle(ctx, &state.gfx.big_circle, region_offset, state.gfx.measures.outer.get_block_size_without_padding())?;
            },
            _ => {
